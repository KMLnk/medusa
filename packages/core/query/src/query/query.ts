...
export class Query {
  ...
  async query(
    ...
    if (Query.traceRemoteQuery) {
      return await Query.traceRemoteQuery(
        async () => await this.#remoteJoiner.query(normalizedQuery, options),
        queryOptions
      )
    }

    return await this.#remoteJoiner.query(normalizedQuery, options)
  }
  ...
  async graph<const TEntry extends string>(
    ...
    if (Query.traceGraphQuery) {
      ...
      response = await this.#remoteJoiner.query(normalizedQuery, options)
    }
